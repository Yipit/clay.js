...
vows.describe('Redis Storage Mechanism').addBatch({
    'by calling *store.persist(instance{zach}, callback)*': {
        topic: function() {
            var topic = this;
            ...
            clear_redis(function(){
                ...
                redis_storage.persist(zach, function(err, keys, zach_array, store, connection){
                    client.hgetall("clay:User:id:1", function(err, data){
                        topic.callback(err, keys, zach_array, store, connection, data);
                    });
                });
            });
        },
        'it increments the index': function(err, keys, zach_array, store, connection, data){
            should.exist(keys);
            ...
            zach_array.should.have.lengthOf(1);
            ...
            zach_array[0].__data__.should.eql({
                __id__: 1,
                name: 'Zach Smith',
                email: 'zach@yipit.com',
                password: '8e5a04ac30cf92eafe36e7a6f9ae9e3af240dc06'
    ...
    'by calling *instance.save({build "worked!"}, callback)*': {
        topic: function() {
            var topic = this;
    ...
    'changing an already saved instance just update the values{ben}': {
        topic : function(){
            var topic = this;
            clear_redis(function(){
                User.create({
    ...
    //             redis_storage.persist(nitya_and_henri, function(err, key, guys, store, connection){
    //                 var items = [];
    ...
    //     },
    // },
    // 'by calling *instance.erase(callback)*': {
    //     topic: function() {
    //         var topic = this;
